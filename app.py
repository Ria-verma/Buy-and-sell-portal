...
def send_otp(reciever, otp):
    msg = Message('OTP', sender='cs207courseproject@gmail.com',
                  recipients=[reciever])
...
def send_otp_for_forgotPassword(reciever, otp):
    msg = Message('OTP', sender='cs207courseproject@gmail.com',
                  recipients=[reciever])
    msg.body = "here is your one time password to reset your account password is :" + \
        str(otp)
    mail.send(msg)
...
def myAccount():
    ...
    if request.method == "POST":
        ...
        cur.execute('UPDATE website.users SET houseno = %s,streetname=%s,city=%s,state=%s,pincode=%s WHERE (email=%s)',
                    (houseno, streetname, city, state, pincode, session["email"]))
...
def addProduct():
    ...
    if request.method == 'POST':
        ...
        cur = mysql.connection.cursor()
        cur.execute("INSERT INTO product(productname, category, description, price, disprice) VALUES(%s, %s, %s, %s, %s)",
                    (productname, category, description, price, disprice))
        mysql.connection.commit()
        cur.close()
